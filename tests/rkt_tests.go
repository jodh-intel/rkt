...
// expectRECommon is a regular-expression variant of expectCommon().
func expectRECommon(p *gexpect.ExpectSubprocess, re string, timeout time.Duration) (lines []string, err error) {

	p.Capture()
	if timeout == 0 {
		lines, err = p.ExpectRegexFind(re)
	} else {
		lines, err = p.ExpectTimeoutRegexFind(re, timeout)
	}
	if err != nil {
		return nil, fmt.Errorf(string(p.Collect()))
	}
...
func expectWithOutputGetAll(p *gexpect.ExpectSubprocess) ([]string, error) {
	// Obtain all output by matching everything
	result, err := expectRECommon(p, "(?s:.*)", 0)
	if err != nil {
		return nil, err
	}
	...
	if l != 1 {
		return nil, fmt.Errorf("expected all output in 1 line, got %d", l)
	}

	lines := strings.Split(string(result[0]), "\n")
	l = len(lines)

	last := lines[l-1]

	if l >= 1 && last == "" {
		// remove last empty line generated by Split
		lines = lines[:l-1]
	}
...
func expectWithOutputArray(p *gexpect.ExpectSubprocess, searchStrings []string) error {
	...
	for _, ss := range searchStrings {
		got := false

		for _, line := range lines {
			if strings.Contains(line, ss) == true {
				got = true
				break
			}
		}
		...
		if got == false {
			return fmt.Errorf("failed to match literal string '%v' in output: %v", ss, strings.Join(lines, "\n"))
		}
	}
...
func expectWithOutputREArray(p *gexpect.ExpectSubprocess, regexps []string) error {
	lines, err := expectWithOutputGetAll(p)
	if err != nil {
		return err
	}
	...
	for _, re := range regexps {
		...
		if got == false {
			return fmt.Errorf("failed to match regular expression '%v' in output: %v", re, strings.Join(lines, "\n"))
		}
	}
...
func runRktAsGidAndCheckOutput(t *testing.T, rktCmd, expectedLine string, expectError bool, gid int) {
	lines := []string{expectedLine}
	runRktAsUidGidAndCheckOutput(t, rktCmd, lines, expectError, nobodyUid, gid)
}

func runRktAsGidAndCheckOutputArray(t *testing.T, rktCmd string, expectedLines []string, expectError bool, gid int) {
	runRktAsUidGidAndCheckOutput(t, rktCmd, expectedLines, expectError, nobodyUid, gid)
}

func runRktAsUidGidAndCheckOutput(t *testing.T, rktCmd string, expectedLines []string, expectError bool, uid, gid int) {
	...
	if expectedLines != nil {
		if err := expectWithOutputArray(child, expectedLines); err != nil {
			t.Fatalf("didn't receive expected output %v: %v", expectedLines, err)
		}
	}
}
